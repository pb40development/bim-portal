 ...
 *
 * This class handles loading configuration from multiple sources with hierarchical search:
 * 1. Environment variables (highest priority)
 * 2. .env file in current directory
 * 3. .env file in parent directory (for subprojects)
...
public class ConfigLoader {
    ...
    /**
     * Find .env file using hierarchical search strategy.
     * Searches in this order:
     * 1. Current working directory
     * 2. Parent directory (one level up)
     * 3. Two levels up (project root for subprojects)
     * 4. Three levels up (just in case)
     *
     ...
     * Priority order:
     * 1. Environment variables (highest priority)
     * 2. .env file (hierarchical search)
     * 3. System properties
    ...
    public static String getProperty(String key, String defaultValue) {
        ...
        if (dotenv != null) {
            ...
            if (value != null && !value.trim().isEmpty()) {
                logger.trace("Found {} in .env file: {}", key, foundEnvFile != null ? foundEnvFile.getAbsolutePath() : "unknown");
                return value.trim();
            }
        }
