...
public class QuickStart {
    ...
    public static EnhancedBimPortalClient quickStart() {
        ...
        try {
            ...
            if (healthCheck.isAuthWorking()) {
                ...
                logger.debug("Full authentication successful. Stats: projects={}, properties={}, loins={}, domainModels={}",
                        stats.getProjectCount(), stats.getPropertyCount(), stats.getLoinCount(), stats.getDomainModelCount());
                ...
                logger.debug("Limited access mode. Authentication failed: {}", healthCheck.getStatusMessage());
            }
    ...
    public static void demonstrateBasicUsage(EnhancedBimPortalClient client) {
        ...
        if (!projects.isEmpty()) {
            ...
            for (int i = 0; i < Math.min(3, projects.size()); i++) {
                ...
                logger.debug("Project {}: name='{}', guid='{}'", i+1, project.getName(), project.getGuid());
            }
        } else {
        ...
        if (!properties.isEmpty()) {
            ...
            for (int i = 0; i < Math.min(3, properties.size()); i++) {
                ...
                logger.debug("Property {}: name='{}', dataType='{}'", i+1, property.getName(), property.getDataType());
            }
        } else {
    ...
    public static void demonstrateExports(EnhancedBimPortalClient client) {
        ...
        if (exportableProject.isPresent()) {
            ...
            logger.debug("Testing export with project: name='{}', guid='{}'", project.getName(), project.getGuid());
            ...
            if (pdfContent.isPresent()) {
                ...
                if (savedFile.isPresent()) {
                    ...
                    logger.debug("Export successful: file='{}', size={} bytes", savedFile.get(), pdfContent.get().length);
                } else {
                    System.out.println("Export succeeded but file save failed");
                    logger.warn("Export content received but file save failed for project '{}'", project.getName());
                }
            } else {
