...
public interface InfrastrukturApi extends ApiClient.Api {
  ...
  /**
   * Liste aller über die Rest-API nutzbaren Organisationen
   * Similar to <code>getOrganisationsForPublic</code> but it also returns the http response headers .
   * 
  ...
  ApiResponse<List<OrganisationForPublicDTO>> getOrganisationsOfUserWithHttpInfo(@Param("userId") UUID userId);


  /**
   * Liste aller über die Rest-API nutzbaren Organisationen, in denen der Nutzer Mitglied ist.
   * Diese API erfordert eine Authentifizierung mittels &#39;Bearer {accessToken}&#39; über den Authorization-Header.
   * Note, this is equivalent to the other <code>getOrganisationsOfUser</code> method,
   * but with the query parameters collected into a single Map parameter. This
   * is convenient for services with optional query parameters, especially when
   * used with the {@link GetOrganisationsOfUserQueryParams} class that allows for
  ...
  List<OrganisationForPublicDTO> getOrganisationsOfUser(@QueryMap GetOrganisationsOfUserQueryParams queryParams);

  /**
  * Liste aller über die Rest-API nutzbaren Organisationen, in denen der Nutzer Mitglied ist.
  * Diese API erfordert eine Authentifizierung mittels &#39;Bearer {accessToken}&#39; über den Authorization-Header.
  * Note, this is equivalent to the other <code>getOrganisationsOfUser</code> that receives the query parameters as a map,
  * but this one also exposes the Http response headers
   ...
   @SuppressWarnings("deprecation")
   ApiResponse<List<OrganisationForPublicDTO>> getOrganisationsOfUserWithHttpInfo(@QueryMap GetOrganisationsOfUserQueryParams queryParams);


   /**
   * A convenience class for generating query parameters for the
   * <code>getOrganisationsOfUser</code> method in a fluent style.
   */
  ...
  ApiResponse<JWTTokenPublicDto> refreshTokenWithHttpInfo(RefreshTokenRequestDTO refreshTokenRequestDTO);
