...
public interface MerkmalsgruppenApi extends ApiClient.Api {
  ...
  /**
   * Liefert die Merkmalsgruppe mit der gesuchten GUID
   * Similar to <code>getPropertyGroupForPublic</code> but it also returns the http response headers .
   * Diese API erlaubt optional eine Authentifizierung mittels &#39;Bearer {accessToken}&#39; über den Authorization-Header, um nicht öffentlich sichtbare Merkmalsgruppen abrufen zu können.
   * @param guid  (required)
  ...
  List<PropertyOrGroupForPublicDto> getPropertyGroupsForPublic(PropertyOrGroupForPublicRequest propertyOrGroupForPublicRequest);

  /**
   * Liefert alle Merkmalsgruppen, die zu den Suchparametern passen
   * Similar to <code>getPropertyGroupsForPublic</code> but it also returns the http response headers .
   * Diese API erlaubt optional eine Authentifizierung mittels &#39;Bearer {accessToken}&#39; über den Authorization-Header, um nicht öffentlich sichtbare Merkmalsgruppen abrufen zu können.
   * @param propertyOrGroupForPublicRequest  (optional)
   * @return A ApiResponse that wraps the response boyd and the http headers.
   */
  ...
  ApiResponse<List<PropertyOrGroupForPublicDto>> getPropertyGroupsForPublicWithHttpInfo(PropertyOrGroupForPublicRequest propertyOrGroupForPublicRequest);
