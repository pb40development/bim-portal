...
public interface MerkmaleApi extends ApiClient.Api {
  ...
  /**
   * Liefert alle globalen Filter zurück
   * Similar to <code>getGlobalFilters</code> but it also returns the http response headers .
   * 
  ...
  List<PropertyOrGroupForPublicDto> getPropertiesForPublic(PropertyOrGroupForPublicRequest propertyOrGroupForPublicRequest);

  /**
   * Liefert alle Merkmale, die zu den Suchparametern passen
   * Similar to <code>getPropertiesForPublic</code> but it also returns the http response headers .
   * Diese API erlaubt optional eine Authentifizierung mittels &#39;Bearer {accessToken}&#39; über den Authorization-Header, um nicht öffentlich sichtbare Merkmale abrufen zu können.
   * @param propertyOrGroupForPublicRequest  (optional)
   * @return A ApiResponse that wraps the response boyd and the http headers.
   */
  ...
  ApiResponse<List<PropertyOrGroupForPublicDto>> getPropertiesForPublicWithHttpInfo(PropertyOrGroupForPublicRequest propertyOrGroupForPublicRequest);
  ...
  /**
   * Liefert das Merkmal mit der gesuchten GUID
   * Similar to <code>getPropertyForPublic</code> but it also returns the http response headers .
   * Diese API erlaubt optional eine Authentifizierung mittels &#39;Bearer {accessToken}&#39; über den Authorization-Header, um nicht öffentlich sichtbare Merkmale abrufen zu können.
   * @param guid  (required)
   * @return A ApiResponse that wraps the response boyd and the http headers.
   */
