...
public interface KontextinformationenApi extends ApiClient.Api {
  ...
  /**
   * Liefert die Kontextinformation mit der übergebenen GUID im OpenOffice-Format zurück
   * Similar to <code>exportOdt2</code> but it also returns the http response headers .
   * Diese API erlaubt optional eine Authentifizierung mittels &#39;Bearer {accessToken}&#39; über den Authorization-Header, um nicht öffentlich sichtbare Informationselemente abrufen zu können.
   * @param guid  (required)
  ...
  /**
   * Liefert die Kontextinformation mit der übergebenen GUID im PDF-Format zurück
   * Similar to <code>exportPdf2</code> but it also returns the http response headers .
   * Diese API erlaubt optional eine Authentifizierung mittels &#39;Bearer {accessToken}&#39; über den Authorization-Header, um nicht öffentlich sichtbare Informationselemente abrufen zu können.
   * @param guid  (required)
   * @return A ApiResponse that wraps the response boyd and the http headers.
   */
  ...
  /**
   * Liefert die Kontextinformation mit der gesuchten GUID
   * Similar to <code>getContextInfoForPublic</code> but it also returns the http response headers .
   * Diese API erlaubt optional eine Authentifizierung mittels &#39;Bearer {accessToken}&#39; über den Authorization-Header, um nicht öffentlich sichtbare Informationselemente abrufen zu können.
   * @param guid  (required)
   * @return A ApiResponse that wraps the response boyd and the http headers.
   */
  ...
  ApiResponse<AIAContextInfoPublicDto> getContextInfoForPublicWithHttpInfo(@Param("guid") UUID guid);
  ...
  List<SimpleContextInfoPublicDto> getContextInfosForPublic(AiaContextInfoPublicRequest aiaContextInfoPublicRequest);

  /**
   * Liefert alle Kontextinformationen, die zu den Suchparametern passen
   * Similar to <code>getContextInfosForPublic</code> but it also returns the http response headers .
   * Diese API erlaubt optional eine Authentifizierung mittels &#39;Bearer {accessToken}&#39; über den Authorization-Header, um nicht öffentlich sichtbare Informationselemente abrufen zu können.
   * @param aiaContextInfoPublicRequest  (required)
   * @return A ApiResponse that wraps the response boyd and the http headers.
   */
  ...
  ApiResponse<List<SimpleContextInfoPublicDto>> getContextInfosForPublicWithHttpInfo(AiaContextInfoPublicRequest aiaContextInfoPublicRequest);
